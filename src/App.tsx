...
export default function App() {
  ...
  const orderStats = useMemo(() => {
    ...
    // RIMOSSO isDateInRange per i "daEvadere", vogliamo vederli sempre finché non vengono spuntati!
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 space-y-4">
                  ...
                  <div className="space-y-3">
                    {(() => {
                      ...
                      const hostessEvents = Object.entries(rubrica).map(([id, d]) => {
                        // Troviamo la rivendita senza curarci dei filtri temporali
                        const riv = allRivendite.find(r => getRivenditaId(r) === id);
                        if (!riv) return null;
                        
                        // Cerchiamo nello storico un evento HOSTESS
                        const history = (d as any).history || [];
                        const lastHostess = history.find((h: any) => h.tipo === 'HOSTESS');
                      ...
                      if (hostessEvents.length === 0) {
                         return <p className="text-[10px] text-slate-400 italic text-center py-4">Nessun servizio Hostess programmato.</p>;
                      }

                      return hostessEvents.map((h: any, i: number) => (
                         <div key={`${h.id}-${i}`} className="p-1.5 bg-purple-50/50 border border-purple-100 rounded-xl">
                            <div className="flex justify-between items-start mb-1">
                              <p className="text-xs font-bold text-slate-800 truncate">{h.nome}</p>
                              <button 
                                onClick={() => { setRivenditaFilter(h.soloNumero); setActiveTab('crm'); }} 
                                className="p-1.5 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors"
                              >
                                <ChevronRight className="w-3 h-3 text-slate-400" />
                              </button>
                            </div>
                            <p className="text-[10px] text-slate-500 mb-2">{h.comune}</p>
                            <div className="bg-white/70 p-1.5 rounded-lg border border-purple-100">
                ...
                <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 space-y-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {orderStats.daEvadere > 0 && (
                      <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-lg text-[10px] font-black border border-blue-200">{orderStats.daEvadere}</span>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    {orderStats.listaDaEvadere.length > 0 ? (
                      orderStats.listaDaEvadere.map((o: any) => (
                        <div key={o.id} className="p-1.5 bg-blue-50/50 border border-blue-100 rounded-xl">
                          <div className="flex justify-between items-start mb-1">
                          ...
                          <div className="bg-white/50 p-1.5 rounded-lg border border-blue-50">
                            <p className="text-[10px] font-medium text-slate-600 italic">Note: {o.note}</p>
                          </div>
                        </div>
                      ))
                    ) : <p className="text-[10px] text-slate-400 italic text-center py-4">Nessun ordine da evadere.</p>}
                  </div>
                </div>
              </div>
