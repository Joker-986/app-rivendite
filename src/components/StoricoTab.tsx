...
import { 
  ...
  Ghost, Phone, MessageCircle, X, ExternalLink, CalendarClock, ListOrdered, Package, TrendingUp, ChevronDown, ChevronUp, Percent, SearchX, Search
} from 'lucide-react';
...
const StoricoTab: React.FC<StoricoTabProps> = ({
  ...
  const listToShow = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      targetList = targetList.filter(item => {
        const nome = item.riv.isStore ? item.riv.storeName : item.riv.Comune;
        const num = item.riv['Num. Rivendita'] || '';
        return (nome && nome.toLowerCase().includes(term)) || (num && String(num).toLowerCase().includes(term));
      });
    }
  ...
  }, [categoriesData, filterMode, sortMode, searchTerm]);

  // Reset del contatore paginazione se si cambiano i filtri o la ricerca
  React.useEffect(() => {
    setVisibleCount(20);
  }, [filterMode, sortMode, searchTerm]);
    ...
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden pb-24 pt-2">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 relative">
        {listToShow.length === 0 ? (
          ...
          <>
            {listToShow.slice(0, visibleCount).map((item, idx) => {
                    ...
                    <div className="flex gap-2">
                      <div className="flex-1 bg-slate-50 rounded-lg p-2 border border-slate-100 flex justify-between items-center">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Ordini</span>
                        </div>
                        <div className="text-xs font-black text-slate-700">
                          {item.count} <span className="text-[9px] font-bold text-slate-400">/ €{item.ltv.toLocaleString('it-IT', { maximumFractionDigits: 0 })}</span>
                        </div>
                      </div>

                      <div className={`flex-1 rounded-lg p-2 border flex justify-between items-center ${sortMode === 'fatturato' ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-100'}`}>
                        <div className="flex items-center gap-1.5">
                          <Wallet className="w-3 h-3 text-slate-400" />
                          <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1">
                            Media {sortMode === 'fatturato' && <TrendingUp className="w-2 h-2 text-emerald-600" />}
                          </span>
                        </div>
                        <div className={`text-xs font-black ${sortMode === 'fatturato' ? 'text-emerald-700' : 'text-slate-700'}`}>
            ...
            {visibleCount < listToShow.length && (
              <div className="pt-2 pb-6 px-1 flex justify-center">
                ...
                >
                  Mostra Altri ({listToShow.length - visibleCount} rimanenti)
                </button>
              </div>
            )}
          </>
